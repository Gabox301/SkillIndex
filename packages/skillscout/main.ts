...
const VERSION: string = (() => {
  for (const base of [__dirname, resolve(__dirname, "..")]) {
    ...
    try {
      ...
      if (typeof pkg.name === "string" && pkg.name.toLowerCase() === "skillscout") return pkg.version;
    } catch {}
  }
  return "0.0.0";
})();
const ISSUES_URL = "https://github.com/GaboTech/skillscout/issues";
...
function showHelp(): void {
  log(`
  ${bold("skillscout")} — Auto-install the best AI skills for your project

  ${bold("Usage:")}
    npx skillscout                   Detect & install skills
    npx skillscout ${dim("-y")}                   Skip confirmation
    npx skillscout ${dim("--dry-run")}            Show what would be installed
    npx skillscout ${dim("--clear-cache")}        Clear downloaded skills cache
    npx skillscout ${dim("-a cursor claude-code")} Install for specific IDEs only
...
function printSummary({ installed, failed, errors, elapsed, verbose }: SummaryOptions): void {
  ...
  if (failed === 0) {
    ...
    if (errors.length > 0) {
      ...
      log(dim(`   If it looks like an skillscout bug, please create an issue: ${ISSUES_URL}`));
    }
  }
...
async function main(): Promise<void> {
  ...
  if (clearCache) {
    ...
    log(
      removed
        ? green(`   ✔ Cleared skillscout cache: ${cacheDir}`)
        : dim(`   No skillscout cache found: ${cacheDir}`),
    );
    log();
    process.exit(0);
  }
  ...
  if (claudeCleanup.cleaned) {
    if (claudeCleanup.deleted) {
      log(dim("   Removed skillscout section from CLAUDE.md (file was empty, deleted)."));
    } else {
