...
// Preserva el probe `dist/main.js` + `--experimental-strip-types` para `main.ts`.
...
if (existsSync(join(__dirname, 'dist', 'main.js'))) {
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      ['--experimental-strip-types', '--disable-warning=ExperimentalWarning', mainPath, ...process.argv.slice(2)],
      { stdio: 'inherit' },
    );
