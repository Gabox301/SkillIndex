...
fn parse_pnpm_workspace_yaml(content: &str) -> Vec<String> {
    ...
    let mut in_packages: bool = false;
...
fn expand_workspace_patterns(project_dir: &Path, patterns: &[String]) -> Vec<PathBuf> {
    ...
    for pattern in patterns {
        if pattern.contains('*') {
            ...
            let parent_rel: &str = pattern
                .rfind('*')
                .map(|idx: usize| {
                    let prefix: &str = &pattern[..idx];
                    prefix
                        .trim_end_matches('/')
                        .trim_end_matches('*')
                        .trim_end_matches('/')
                ...
                .unwrap_or("");
            let parent: PathBuf = if parent_rel.is_empty() {
            ...
            for entry in entries.flatten() {
                let file_type: fs::FileType = match entry.file_type() {
                    Ok(ft) => ft,
                    Err(_) => continue,
                };
                if !file_type.is_dir() {
                ...
                let ws_dir: PathBuf = parent.join(&name);
            ...
            let ws_dir: PathBuf = project_dir.join(pattern);
            if ws_dir.join("package.json").exists()
                || ws_dir.join("deno.json").exists()
                || ws_dir.join("deno.jsonc").exists()
            {
...
fn read_package_json_workspaces(project_dir: &Path) -> Option<Vec<String>> {
    let data: String = fs::read_to_string(project_dir.join("package.json")).ok()?;
    let v: serde_json::Value = serde_json::from_str(&data).ok()?;
    let ws: &serde_json::Value = v.get("workspaces")?;
    if let Some(arr) = ws.as_array() {
        ...
        && let Some(arr) = obj
            .get("packages")
            .and_then(|x: &serde_json::Value| x.as_array())
    {
        let mut out: Vec<String> = Vec::new();
        for item in arr {
            if let Some(s) = item.as_str() {
...
pub fn resolve_workspaces(project_dir: &Path) -> Vec<PathBuf> {
    ...
    let pnpm_path: PathBuf = project_dir.join("pnpm-workspace.yaml");
    if pnpm_path.exists()
        && let Ok(content) = fs::read_to_string(&pnpm_path)
    {
        let patterns: Vec<String> = parse_pnpm_workspace_yaml(&content);
    ...
    if let Some(patterns) = read_package_json_workspaces(project_dir) {
        return expand_workspace_patterns(project_dir, &patterns)
            .into_iter()
            .filter(|p: &PathBuf| p.canonicalize().ok() != project_dir.canonicalize().ok())
            .collect();
    }
...
mod tests {
    ...
    fn detects_npm_workspaces_array_format() {
        ...
        let result: Vec<PathBuf> = resolve_workspaces(dir.path());
