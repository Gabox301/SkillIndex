...
describe("installSkill", () => {
  ...
  it("copies files to .agents/skills/<name> and updates lock", async () => {
    ...
    equal(lock.skills["hello-skill"].sourceType, "skillscout-registry");
  ...
  it("downloads from the raw GitHub registry by default", async () => {
    ...
    try {
      result = await installSkill("owner/repo/raw-skill", [], {
        ...
        fetchImpl: (async (url: string | URL | Request) => {
          ...
          ok(
            href.startsWith(
              `https://raw.githubusercontent.com/GaboTech/skillscout/v${PACKAGE_VERSION}/`,
            ),
          );
    ...
    ok(
      trace.some((line) =>
        line.includes(
          `downloaded AGENTS.md from https://raw.githubusercontent.com/GaboTech/skillscout/v${PACKAGE_VERSION}/packages/skillscout/skills-registry/raw-skill/AGENTS.md`,
        ),
      ),
    );
    equal(
    ...
    try {
      const result = await installSkill("owner/repo/fallback-skill", [], {
        ...
        fetchImpl: (async (url: string | URL | Request) => {
          const href = typeof url === "string" || url instanceof URL ? String(url) : url.url;
          seenUrls.push(href);
          if (href.includes(`/GaboTech/skillscout/v${PACKAGE_VERSION}/`)) {
            return new Response("not found", { status: 404, statusText: "Not Found" });
          }
          if (href.includes("/GaboTech/skillscout/main/packages/skillscout/skills-registry/")) {
            const rel = decodeURIComponent(
              href.split("/GaboTech/skillscout/main/packages/skillscout/skills-registry/")[1],
            );
      ...
      ok(
        seenUrls.some((url) =>
          url.startsWith(
            "https://raw.githubusercontent.com/GaboTech/skillscout/main/packages/skillscout/skills-registry/fallback-skill/",
          ),
        ),
      );
      equal(
