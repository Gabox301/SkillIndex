...
pub fn format_detected(
    ...
    if !detected.is_empty() {
        ...
        out.push_str(&format!(
            "{}\n",
            cyan("   ◆ ") + &bold("Detected technologies:")
        ));
        out.push('\n');
    ...
    if is_frontend && detected.is_empty() {
        out.push_str(&format!(
            "{}\n",
            cyan("   ◆ ") + &bold("Web frontend detected ") + &dim("(from project files)")
        ));
        out.push('\n');
    }
...
pub fn format_skills_list(skills: &[SkillEntry]) -> String {
    ...
    let count_label = if installed_count > 0 {
        format!("({}, {} already installed)", skills.len(), installed_count)
    } else {
        format!("({})", skills.len())
    };
    ...
    out.push_str(&format!(
        "{}\n",
        cyan("   ◆ ") + &bold("Skills to install ") + &dim(&count_label)
    ));
    out.push('\n');
...
pub fn format_security_checks(checks: &[InstallSecurityCheck]) -> String {
    ...
    out.push_str(&format!(
        ...
        dim(&format!(
            ...
            visible_pad("Check", check_width),
            visible_pad("Findings", findings_width)
        ))
    ));
    out.push_str(&format!(
        "{}\n",
