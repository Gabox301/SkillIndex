...
pub fn get_registry_raw_base_urls(registry_base_url: Option<&str>) -> Vec<String> {
    ...
    let base = "https://raw.githubusercontent.com/GaboTech/skillscout";
    vec![
        format!("{base}/v{version}/packages/skillscout/skills-registry"),
        format!("{base}/main/packages/skillscout/skills-registry"),
    ]
}
...
pub fn get_registry_dir() -> PathBuf {
    ...
    if let Some(parent) = manifest_dir.parent() {
        ...
        // also try parent/packages/skillscout/skills-registry
        let c3 = parent
            .join("packages")
            .join("skillscout")
...
pub fn verify_registry_entry(
    ...
    if !skill_dir.exists() {
        return VerifyResult {
            ok: false,
            reason: Some(format!("missing directory {}", skill_dir.display())),
        };
    }
    for rel in &entry.files {
        ...
        if !abs.exists() {
            return VerifyResult {
                ok: false,
                reason: Some(format!("missing file {normalized}")),
            };
        }
        let expected = entry
            .sha256
        ...
        let Some(expected) = expected else {
            return VerifyResult {
                ok: false,
                reason: Some(format!("no recorded hash for {normalized}")),
            };
        };
        let actual = match sha256_file(&abs) {
            ...
            Err(e) => {
                return VerifyResult {
                    ok: false,
                    reason: Some(format!("failed to hash {normalized}: {e}")),
                };
            }
        };
        if &actual != expected {
            return VerifyResult {
                ok: false,
                reason: Some(format!("hash mismatch for {normalized}")),
            };
        }
    }
    VerifyResult {
...
pub fn security_check_for_entry(skill_name: &str, entry: &RegistryEntry) -> InstallSecurityCheck {
    ...
    let summary = if !entry.review.summary.is_empty() {
        ...
        "The sync review found issues that should be checked.".to_string()
    } else {
        "The sync review did not find security issues.".to_string()
    };
...
mod tests {
    ...
    fn verify_registry_entry_missing_dir() {
        ...
        let result = verify_registry_entry("my-skill", &entry, &reg_dir);
        assert!(!result.ok);
        assert!(result.reason.unwrap().contains("missing directory"));
    }
    ...
    fn security_check_falls_back_to_review_approved() {
        ...
        assert!(check.summary.contains("did not find"));
    }
    ...
    fn get_registry_raw_base_urls_default() {
        ...
        assert!(urls[1].ends_with("/main/packages/skillscout/skills-registry"));
        if let Some(v) = prev1 {
