...
const DEFAULT_REGISTRY_RAW_BASE_URL_PREFIX = "https://raw.githubusercontent.com/GaboTech/skillscout";
...
function getRegistryRawBaseUrls(opts: InstallOptions): string[] {
  ...
  if (!version) {
    throw new Error("could not resolve skillscout package version for registry download");
  }

  const base = DEFAULT_REGISTRY_RAW_BASE_URL_PREFIX;
  return [
    `${base}/v${version}/packages/skillscout/skills-registry`,
    `${base}/main/packages/skillscout/skills-registry`,
  ];
}
...
export function getSkillScoutCacheDir(): string {
  return (
    ...
    join(homedir(), ".cache", "skillscout", "skills-registry")
  );
}
...
function githubDownloadHeaders(url: string): HeadersInit {
  const headers: Record<string, string> = { "User-Agent": "skillscout" };
...
export async function installSkill(
  ...
  const command = `skillscout install ${skillPath}`;
