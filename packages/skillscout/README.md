...
`skillscout` escanea tu proyecto, detecta las tecnologías que usas e instala skills curadas de [agentes IA](https://skills.sh) que hacen que Cursor, Claude Code y otros asistentes realmente entiendan tu stack.
...
```bash
npx skillscout --dry-run
```

### Resumen para Claude Code

Si `claude-code` es autodetectado o se pasa con `-a`, `skillscout` escribe un archivo `CLAUDE.md` en la raíz de tu proyecto resumiendo los archivos markdown instalados bajo `.claude/skills`.
...
`skillscout` detecta **más de 50 tecnologías** desde tu `package.json`, lockfiles, archivos Gradle y archivos de configuración:
...
Incluso sin framework, `skillscout` escanea tu árbol de archivos en busca de señales de frontend web (`.html`, `.css`, `.scss`, `.vue`, `.svelte`, `.jsx`, `.tsx`, `.twig`, `.blade.php`, etc.) e instala skills de diseño frontend, accesibilidad y SEO.

## Detección de combinaciones

Cuando se usan múltiples tecnologías juntas, `skillscout` detecta **combinaciones de tecnologías** y añade skills especializadas para la combinación:
...
`skillscout` usa [skills.sh](https://skills.sh) por debajo — el registro abierto de skills para agentes IA. Las skills son archivos markdown que enseñan a los asistentes IA cómo trabajar con tecnologías específicas, siguiendo mejores prácticas y patrones de los mantenedores oficiales.
...
MIT — Creado por [GaboTech](https://github.com/Gabox301)
