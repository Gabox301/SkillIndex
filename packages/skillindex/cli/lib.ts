...
export type { ComboSkill, ConfigFileContentBlock, Technology } from '../skills-map.ts';
...
const DOTNET_SCAN_ROOT_FILES = ['global.json', 'NuGet.Config', 'Directory.Build.props', 'Directory.Packages.props'];
...
function hasFileWithExtension(projectDir: string, extensions: string[], maxDepth: number = 4): boolean {
...
function detectTechnologiesInDir(
  dir: string,
  { skipFrontendFiles = false, pkg: preloadedPkg, denoJson: preloadedDeno }: DetectInDirOptions = {},
): DetectInDirResult {
...
export function collectSkills({
  ...
  function addSkill(skill: string, source: string): void {
    ...
    if (!existing) {
      const installed = installedNames ? installedNames.has(parseSkillPath(skill).skillName) : false;
