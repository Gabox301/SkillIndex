...
pub fn github_download_headers(url: &str) -> reqwest::header::HeaderMap {
    ...
    headers.insert(
        ...
        reqwest::header::HeaderValue::from_static("skillscout"),
    );
...
pub fn update_skills_lock(
    ...
    lock["skills"][skill_name] = serde_json::json!({
        "source": entry.source,
        "sourceType": "skillscout-registry",
...
async fn download_registry_file(
    ...
    if is_disallowed_skill_file(&normalized) {
        return Err(format!(
            "refusing to download disallowed skill archive: {normalized}"
        ));
    }
    let expected = entry
        .sha256
        ...
        .ok_or_else(|| format!("no recorded hash for {normalized}"))?
        .clone();
    ...
    for base in &base_urls {
        ...
        if !res.status().is_success() {
            ...
            if status == 403 && remaining == "0" {
                ...
                return Err(format!(
                    "GitHub rate limit exceeded (resets {iso}). Set GITHUB_TOKEN or GH_TOKEN to increase the limit."
                ));
            }
            let msg = format!(
                "{} {} from {}",
        ...
        if actual != expected {
            errors.push(format!("hash mismatch from {base}"));
            continue;
        }
    ...
    Err(format!(
        "download failed for {normalized}: {}",
        errors.join("; ")
    ))
}
...
pub async fn install_skill_with_client(
    ...
    let command = format!("skillscout install {skill_path}");
    ...
    if parsed.skill_name.is_empty() {
        return fail(format!("invalid skill path: {skill_path}"));
    }
    ...
    let Some(registry) = registry else {
        return fail(
            "skills-registry index not found. Run 'pnpm sync:skills' in the skillscout package."
                .to_string(),
        );
    };
    let Some(entry) = registry.skills.get(&skill_name) else {
        return fail(format!(
            "skill '{skill_name}' not found in registry (unaudited)."
        ));
    };
            ...
            match download_registry_entry_to_cache(&skill_name, entry, opts, client).await {
                Ok(cached_skill_dir) => {
                    ...
                    if let Err(e) = copy_dir(&cached_skill_dir, &canonical_dir) {
                        return fail(format!("download failed: {e}"));
                    }
                }
                Err(e) => {
                    return fail(format!("download failed: {e}"));
    ...
    if let Err(e) = update_skills_lock(&project_dir, &skill_name, entry) {
        return fail(format!("lockfile update failed: {e}"));
    }
...
mod tests {
    ...
    async fn install_skill_rejects_zip() {
        ...
        assert!(result
            .output
            .contains("refusing to download disallowed skill archive"));
        match prev_cache {
    ...
    async fn install_skill_not_found() {
        ...
        assert!(!result.success);
        assert!(result.output.contains("not found in registry"));
    }
    ...
    fn is_githubusercontent_detection() {
        ...
        assert!(is_githubusercontent_url(
            "https://raw.githubusercontent.com/GaboTech/skillscout/main/file"
        ));
        assert!(!is_githubusercontent_url(
