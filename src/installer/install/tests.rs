...
async fn install_skill_from_local_registry() {
    ...
    let skill_name: &str = "hello-skill";
    let files: Vec<(&str, &str)> = vec![("SKILL.md", "# hello"), ("references/notes.md", "notes")];
    let entry: RegistryEntry = make_entry(skill_name, files.clone());
    // write registry files
    for (rel, content) in &files {
        let mut p: std::path::PathBuf = reg_dir.join(skill_name);
    ...
    let result: crate::installer::InstallResult =
        install_skill_with_client("owner/repo/hello-skill", &[], &opts, &client).await;
...
async fn install_skill_rejects_zip() {
    ...
    fs::create_dir_all(&project_dir).unwrap();
    fs::create_dir_all(&reg_dir).unwrap();
    let skill_name: &str = "archive-skill";
    // Create entry with .zip file but no file on disk (registry dir removed)
    let entry: RegistryEntry = make_entry(skill_name, vec![("downloads/tool.ZIP", "zipcontent")]);
    ...
    let prev_cache: Option<String> = env::var("SKILLINDEX_CACHE_DIR").ok();
    unsafe {
        env::set_var(
...
async fn install_skill_not_found() {
    ...
    let client: reqwest::Client = reqwest::Client::new();
    let result: crate::installer::InstallResult =
        install_skill_with_client("owner/repo/unknown", &[], &opts, &client).await;
    assert!(!result.success);
...
async fn install_skill_via_httpmock_network() {
    let _env_guard: crate::infra::cache::EnvGuard = crate::infra::cache::env_lock();
    let server: httpmock::prelude::MockServer = httpmock::MockServer::start();
    let tmp: tempfile::TempDir = tempdir().unwrap();
    let reg_dir: std::path::PathBuf = tmp.path().join("registry");
    let project_dir: std::path::PathBuf = tmp.path().join("project");
    fs::create_dir_all(&project_dir).unwrap();
    ...
    let skill_name: &str = "net-skill";
    let content: &str = "# net skill content";
    let entry: RegistryEntry = make_entry(skill_name, vec![("SKILL.md", content)]);
    ...
    let cache_root: std::path::PathBuf = tmp.path().join("cache-net");
    let prev_cache: Option<String> = env::var("SKILLINDEX_CACHE_DIR").ok();
    ...
    let client: reqwest::Client = reqwest::Client::new();
    let result: crate::installer::InstallResult =
        install_skill_with_client("owner/repo/net-skill", &[], &opts, &client).await;
    assert!(
        result.success,
...
async fn install_skill_rate_limit_aborts_with_iso() {
    ...
    let cache_root: std::path::PathBuf = tmp.path().join("cache-rate");
    let prev_cache: Option<String> = env::var("SKILLINDEX_CACHE_DIR").ok();
    unsafe { env::set_var("SKILLINDEX_CACHE_DIR", cache_root.to_str().unwrap()) };
...
async fn install_skill_cache_hit_no_fetch() {
    ...
    let reg_dir: std::path::PathBuf = tmp.path().join("registry");
    let project_dir: std::path::PathBuf = tmp.path().join("project");
    let cache_root: std::path::PathBuf = tmp.path().join("cache-hit");
    fs::create_dir_all(&project_dir).unwrap();
    fs::create_dir_all(&reg_dir).unwrap();
    fs::create_dir_all(&cache_root).unwrap();

    let skill_name: &str = "cached-skill";
    let content: &str = "# cached";
    let entry: RegistryEntry = make_entry(skill_name, vec![("SKILL.md", content)]);
    ...
    let cache_skill_dir: std::path::PathBuf = cache_root.join(&entry.bundle_hash).join(skill_name);
...
async fn install_skill_skips_download_when_already_installed() {
    ...
    let skill_name: &str = "already-skill";
    let content: &str = "# already";
    let entry: RegistryEntry = make_entry(skill_name, vec![("SKILL.md", content)]);
    ...
    let canonical: std::path::PathBuf = project_dir.join(".agents/skills").join(skill_name);
...
async fn install_skill_hash_mismatch_falls_to_second_base() {
    let _env_guard: crate::infra::cache::EnvGuard = crate::infra::cache::env_lock();
    let server1: httpmock::prelude::MockServer = httpmock::MockServer::start();
    let server2: httpmock::prelude::MockServer = httpmock::MockServer::start();
    let tmp: tempfile::TempDir = tempdir().unwrap();
    let reg_dir: std::path::PathBuf = tmp.path().join("registry");
    let project_dir: std::path::PathBuf = tmp.path().join("project");
    fs::create_dir_all(&project_dir).unwrap();
    ...
    let skill_name: &str = "mismatch-skill";
    let good_content: &str = "good content";
    let bad_content: &str = "bad content";
    let entry: RegistryEntry = make_entry(skill_name, vec![("SKILL.md", good_content)]);
    ...
    let mock1: httpmock::Mock<'_> = server1.mock(|when: httpmock::When, then: httpmock::Then| {
        when.method(httpmock::Method::GET)
            .path(format!("/{}/SKILL.md", skill_name));
        then.status(200).body(bad_content);
    });
    // server2 returns good content
    let mock2: httpmock::Mock<'_> = server2.mock(|when: httpmock::When, then: httpmock::Then| {
        when.method(httpmock::Method::GET)
            .path(format!("/{}/SKILL.md", skill_name));
        then.status(200).body(good_content);
    });

    let cache_root: std::path::PathBuf = tmp.path().join("cache-mismatch");
    let prev_cache: Option<String> = env::var("SKILLINDEX_CACHE_DIR").ok();
    unsafe { env::set_var("SKILLINDEX_CACHE_DIR", cache_root.to_str().unwrap()) };
...
async fn install_all_concurrent_sorts_by_repo() {
    ...
    let skills_data: Vec<(&str, Vec<(&str, &str)>)> = vec![
        ("skill-b", vec![("SKILL.md", "b")]),
        ("skill-a", vec![("SKILL.md", "a")]),
    ];
    let mut map: HashMap<String, RegistryEntry> = HashMap::new();
    for (name, files) in &skills_data {
        // write to registry dir for local copy
        for (rel, content) in files {
            let mut p: std::path::PathBuf = reg_dir.join(*name);
            for part in rel.split('/') {
                p = p.join(part);
            }
            fs::create_dir_all(p.parent().unwrap()).unwrap();
    ...
    let client: reqwest::Client = reqwest::Client::new();
    let entries: Vec<SkillEntry> = vec![
        SkillEntry {
    ...
    let result: crate::installer::InstallAllResult =
        install_all_with_client(entries, &[], &opts, &client).await;
