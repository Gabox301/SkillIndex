...
`,
  ...
  it('detects Java from nested Gradle module declared in settings.gradle.kts', () => {
    ...
    writeFile(tmp.path, 'adapters/web/build.gradle.kts', 'sourceCompatibility = JavaVersion.VERSION_17');
    const { detected } = detectTechnologies(tmp.path);
  ...
  it('detects Laravel from composer.json with laravel/framework', () => {
    writeFile(tmp.path, 'composer.json', JSON.stringify({ require: { 'laravel/framework': '^11.0' } }));
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === 'laravel'));
  });

  it('detects Laravel from composer.json with illuminate packages', () => {
    writeFile(tmp.path, 'composer.json', JSON.stringify({ require: { 'illuminate/support': '^11.0' } }));
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === 'laravel'));
  });
  ...
  it('detects Chrome Extension from manifest.json with manifest_version', () => {
    writeFile(tmp.path, 'manifest.json', JSON.stringify({ manifest_version: 3, name: 'My Extension', version: '1.0' }));
    const { detected } = detectTechnologies(tmp.path);
    ok(detected.some((t) => t.id === 'chrome-extension'));
  });

  it('does not detect Chrome Extension from manifest.json without manifest_version', () => {
    writeFile(tmp.path, 'manifest.json', JSON.stringify({ name: 'My PWA', short_name: 'PWA', start_url: '/' }));
    const { detected } = detectTechnologies(tmp.path);
    ok(!detected.some((t) => t.id === 'chrome-extension'));
  });
  ...
  it('detects chrome-extension-clerk combo end-to-end', () => {
    ...
    writeFile(tmp.path, 'manifest.json', JSON.stringify({ manifest_version: 3, name: 'Ext', version: '1.0' }));
    const { combos } = detectTechnologies(tmp.path);
...
describe('detectCombos', () => {
  ...
  it('detects combo even with extra technologies', () => {
    ok(detectCombos(['react', 'expo', 'tailwind', 'typescript']).some((c) => c.id === 'expo-tailwind'));
  });
  ...
  it('detects react-hook-form-zod combo', () => {
    const combo = detectCombos(['react-hook-form', 'zod']).find((c) => c.id === 'react-hook-form-zod');
    ok(combo);
  ...
  it('detects react-react-three-fiber combo', () => {
    ok(detectCombos(['threejs', 'react', '@react-three/fiber']).some((c) => c.id === 'react-react-three-fiber'));
  });

  it('does not detect react-react-three-fiber combo without react', () => {
    ok(!detectCombos(['threejs', '@react-three/fiber']).some((c) => c.id === 'react-react-three-fiber'));
  });

  it('does not detect react-react-three-fiber combo without Three.js', () => {
    ok(!detectCombos(['react', '@react-three/fiber']).some((c) => c.id === 'react-react-three-fiber'));
  });
