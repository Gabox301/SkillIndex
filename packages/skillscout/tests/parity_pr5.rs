...
use skillscout::cache::get_cache_registry_dir;
use skillscout::detect::{detect_technologies, DetectResult};
...
use skillscout::dotnet::dotnet_layout_candidate_paths;
use skillscout::frontend::has_web_frontend_files;
use skillscout::gradle::{gradle_layout_candidate_paths, parse_settings_gradle_modules};
use skillscout::hash::{
    bundle_hash, is_disallowed_skill_file, normalize_registry_rel_path, sha256_buffer,
};
use skillscout::installer::{
    copy_dir, encode_raw_path, ensure_symlink_to, rel_path_from_to, update_skills_lock,
};
use skillscout::registry::{
    agent_folder_for, get_registry_raw_base_urls, parse_skill_path, Registry,
};
use skillscout::workspace::resolve_workspaces;
use std::fs;
...
fn parity_installer_lock_sorted_newline() {
    ...
    let mut entry = skillscout::registry::RegistryEntry {
        ...
        review: skillscout::registry::Review {
...
fn parity_registry_base_urls_default() {
    ...
    assert!(urls
        .iter()
        .any(|u| u.contains("main/packages/skillscout/skills-registry")));
}
...
fn parity_args_dry_run_flag() {
    ...
    let args = Args::try_parse_from(["skillscout", "--dry-run"]).unwrap();
    assert!(args.dry_run);
}
...
fn parity_args_yes_and_agent() {
    let args =
        Args::try_parse_from(["skillscout", "-y", "-a", "cursor", "-a", "claude-code"]).unwrap();
    assert!(args.yes);
...
fn parity_display_three_col() {
    ...
    let plain = skillscout::ui::strip_ansi(&out);
    for i in 0..7 {
...
fn parity_display_security_sorted() {
    let checks = vec![
        skillscout::registry::InstallSecurityCheck {
...
fn parity_cli_seven_techs_via_binary() {
    ...
    let output = cmd
        ...
        .unwrap();
    let stdout = String::from_utf8_lossy(&output.stdout);
    let plain = skillscout::ui::strip_ansi(&stdout);
    for name in [
        "React",
        "Next.js",
        "Vue",
...
// SKILLSCOUT_USE_RUST=0 fallback — ensure Node index.mjs still works via node
#[test]
fn parity_fallback_node_via_env() {
    // Spawn node index.mjs --help with SKILLSCOUT_USE_RUST=0, should succeed via Node fallback
    ...
    assert!(
        combined.contains("--dry-run") || combined.contains("skillscout"),
...
async fn parity_installer_rate_limit_iso() {
    ...
    use skillscout::hash::sha256_buffer;
    use skillscout::installer::{install_skill_with_client, InstallOptions};
    use skillscout::registry::{Registry, RegistryEntry, Review, Reviewer};
    ...
    let prev = std::env::var("SKILLSCOUT_CACHE_DIR").ok();
    unsafe { std::env::set_var("SKILLSCOUT_CACHE_DIR", cache_root.to_str().unwrap()) };
    ...
    mock.assert();
    match prev {
        Some(v) => unsafe { std::env::set_var("SKILLSCOUT_CACHE_DIR", v) },
        None => unsafe { std::env::remove_var("SKILLSCOUT_CACHE_DIR") },
    }
}
...
fn parity_display_format_time_various() {
    use skillscout::display::format_time;
