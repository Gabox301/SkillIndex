...
fn add_workspace(root: &Path, ws: &str) {
    let p: std::path::PathBuf = root.join(ws);
...
fn ids(r: &DetectResult) -> Vec<&str> {
    r.detected
        .iter()
        .map(|t: &skillindex::display::DisplayTechnology| t.id.as_str())
        .collect()
}
...
fn returns_empty_when_no_package_json_or_config_files() {
    ...
    let r: DetectResult = detect_technologies(dir.path());
...
fn rust_detection_returns_correct_skills() {
    ...
    let r: DetectResult = detect_technologies(dir.path());
    let rust: &skillindex::display::DisplayTechnology = r
        .detected
        .iter()
        .find(|t: &&skillindex::display::DisplayTechnology| t.id == "rust")
        .unwrap();
    assert!(
        rust.skills
            .iter()
...
fn clerk_detection_returns_correct_skills() {
    ...
    write_pkg(dir.path(), r#"{"dependencies":{"@clerk/nextjs":"^6"}}"#);
    let r: DetectResult = detect_technologies(dir.path());
    let clerk: &skillindex::display::DisplayTechnology = r
        .detected
        .iter()
        .find(|t: &&skillindex::display::DisplayTechnology| t.id == "clerk")
        .unwrap();
    assert!(
        clerk
            .skills
...
fn deduplicates_technologies_across_workspaces() {
    ...
    let count: usize = r
        .detected
        .iter()
        .filter(|t: &&skillindex::display::DisplayTechnology| t.id == "react")
        .count();
