...
use crate::infra::hash::{normalize_registry_rel_path, sha256_file};

pub fn get_registry_raw_base_urls(registry_base_url: Option<&str>) -> Vec<String> {
    if let Some(url) = registry_base_url {
        let trimmed: String = url.trim_end_matches('/').to_string();
        if !trimmed.is_empty() {
            return vec![trimmed];
        }
    }
...
pub fn get_registry_dir() -> PathBuf {
    ...
    let c1: PathBuf = manifest_dir.join("skills-registry");
    if c1.join("index.json").exists() {
        return c1;
    }
    if let Some(parent) = manifest_dir.parent() {
        let c2: PathBuf = parent.join("skills-registry");
        if c2.join("index.json").exists() {
            return c2;
        }
        let c3: PathBuf = parent
            .join("packages")
            .join("skillindex")
    ...
    if let Ok(cwd) = env::current_dir() {
        let c: PathBuf = cwd.join("skills-registry");
    ...
    {
        let c: PathBuf = dir.join("skills-registry");
        if c.join("index.json").exists() {
            return c;
        }
...
pub fn load_registry_from_dir(dir: &Path) -> Option<Registry> {
    let p: PathBuf = dir.join("index.json");
    let data: String = fs::read_to_string(&p).ok()?;
...
pub fn verify_registry_entry(
    ...
    for rel in &entry.files {
        ...
        let abs: PathBuf = {
            let mut p: PathBuf = skill_dir.clone();
        ...
        let actual: String = match sha256_file(&abs) {
            Ok(h) => h,
            Err(e) => {
                return VerifyResult {
                    ok: false,
...
pub fn security_check_for_entry(skill_name: &str, entry: &RegistryEntry) -> InstallSecurityCheck {
    ...
    let status: &str = if entry.review.status == "flagged" {
        "warning"
    } else {
        "ok"
    };
    let summary: String = if !entry.review.summary.is_empty() {
...
mod tests {
    ...
    fn sample_entry(files: Vec<(&str, &str)>) -> RegistryEntry {
        ...
        for (rel, content) in &files {
            let hash: String = crate::infra::hash::sha256_buffer(content.as_bytes());
    ...
    fn write_skill_files(dir: &Path, skill_name: &str, files: &[(&str, &str)]) {
        for (rel, content) in files {
            let normalized: String = normalize_registry_rel_path(rel);
            let mut p: PathBuf = dir.join(skill_name);
            for part in normalized.split('/') {
                p = p.join(part);
            }
    ...
    fn load_registry_from_dir_success() {
        ...
        .unwrap();
        let reg: Registry = load_registry_from_dir(&reg_dir).unwrap();
    ...
    fn verify_registry_entry_ok() {
        ...
        let reg_dir: PathBuf = tmp.path().join("registry");
        fs::create_dir_all(&reg_dir).unwrap();
        let entry: RegistryEntry = sample_entry(vec![("SKILL.md", "# hi")]);
        write_skill_files(&reg_dir, "my-skill", &[("SKILL.md", "# hi")]);
        let result: VerifyResult = verify_registry_entry("my-skill", &entry, &reg_dir);
    ...
    fn verify_registry_entry_missing_file() {
        ...
        let reg_dir: PathBuf = tmp.path().join("registry");
        fs::create_dir_all(&reg_dir).unwrap();
        let mut entry: RegistryEntry = sample_entry(vec![("SKILL.md", "# hi")]);
    ...
    fn verify_registry_entry_backslash_normalized() {
        ...
        let reg_dir: PathBuf = tmp.path().join("registry");
        fs::create_dir_all(&reg_dir).unwrap();
        let mut entry: RegistryEntry = sample_entry(vec![("references\\notes.md", "notes")]);
        entry.files = vec!["references\\notes.md".to_string()];
        write_skill_files(&reg_dir, "win-skill", &[("references/notes.md", "notes")]);
        let result: VerifyResult = verify_registry_entry("win-skill", &entry, &reg_dir);
    ...
    fn get_registry_raw_base_urls_default() {
        let _guard: crate::infra::cache::EnvGuard = crate::infra::cache::env_lock();
        unsafe {
    ...
    fn get_registry_raw_base_urls_with_custom() {
        let urls: Vec<String> =
            get_registry_raw_base_urls(Some("https://example.test/skills-registry/"));
    ...
    fn get_registry_raw_base_urls_env_precedence() {
        let _guard: crate::infra::cache::EnvGuard = crate::infra::cache::env_lock();
        let prev: Option<String> = env::var("SKILLINDEX_REGISTRY_BASE_URL").ok();
    ...
    fn get_registry_dir_finds_fixture() {
        let dir: PathBuf = get_registry_dir();
        assert!(
